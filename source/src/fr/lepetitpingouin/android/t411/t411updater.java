...
public class t411updater extends Service{
	...
	@Override
	 public int onStartCommand(Intent intent, int flags, int startId) {
	  ...
	  if(prefs.getBoolean("updAlert", false))
		  createNotify(1992, R.drawable.ic_stat_updating,this.getString(R.string.notif_update),this.getString(R.string.notif_upd_title),this.getString(R.string.notif_upd_content),false, t411updater.class);
	  
	  try{
		  // Détection du changement de configuration utilisateur : si la config a changé, on dégomme le cookie
		update(prefs.getString("login", ""),prefs.getString("password", ""));
	  }
	  catch(Exception ex){
		  Log.v("Credentials :",prefs.getString("login", "")+":"+prefs.getString("password", ""));
	  ...
	  // on définit une instance de Calendar selon la configuration utilisateur (15 mn par défaut)
	...
	public void update(String login, String password) throws IOException
	{
		...
		Document doc = null;
			// on exécute la requête HTTP, en passant le login et le password en POST.
			...
			if(conError != ""){
				Toast.makeText(getApplicationContext(), conError, Toast.LENGTH_LONG).show();
			}
			else {
				...
				// on récupère la chaine de l'upload sans la traiter, avec la flèche et l'unité
				...
				mails = Integer.valueOf(doc.select(".mail  > strong").first().text());
	...
	public void doNotify()
	{
		if(prefs.getBoolean("ratioAlert", false))
			if(ratio < Double.valueOf(prefs.getString("ratioMinimum", "0")))
				createNotify(1990, R.drawable.ic_stat_ratio,this.getString(R.string.notif_ratio),this.getString(R.string.notif_ratio_title)+" (< "+prefs.getString("ratioMinimum", "???")+")",this.getString(R.string.notif_ratio_content),true, t411updater.class);
			else cancelNotify(1990);
		if(prefs.getBoolean("mailAlert", false))
			if(mails > oldmails)
				createNotify(1991, R.drawable.ic_stat_message,this.getString(R.string.notif_message),this.getString(R.string.notif_msg_title),this.getString(R.string.notif_msg_content),true,messagesActivity.class);
			else cancelNotify(1991);			
	}
	
	private void createNotify(int number, int icon, String label, String title, String description, boolean vibrate, Class <?> cls){
    	NotificationManager notificationManager = (NotificationManager)getSystemService(Context.NOTIFICATION_SERVICE);        
 
    	Notification notification = new Notification(icon, label, System.currentTimeMillis());  
 
    	if(cls!=null)
    		pendingIntent = PendingIntent.getActivity(this, 0, new Intent(this, cls), 0);
